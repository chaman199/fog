...
function highChartFunc(cityid) {
    ...
    Highcharts.getJSON('apis/database.php?station=' + cityid, function(data) {
        ...
        $.ajax({url: "apis/predict.php?stime="+d+"&station="+cityid+"&model=3", success: function(result){
            console.log(result);
          }});
        var current = []
        current.push(fogdata[dataLength - 1][1].toFixed(2));
        current.push(temperaturedata[dataLength - 1][1].toFixed(2))
        ...
        document.getElementById("data-detail").innerHTML = "<h3><div>Current Values: </div><div>Visibility: " + fogdata[dataLength - 1][1].toFixed(2) + " km   <\div><div>Temperature: " + temperaturedata[dataLength - 1][1].toFixed(2) + "°C    <\div><div>Relative Humidity: " + humiditydata[dataLength - 1][1].toFixed(2) + " %    <\div><div>Wind: " + winddata[dataLength - 1][1].toFixed(2) + "Km/h " + winddata[dataLength - 1][2] + "°N" + " <\div></h3>   ";
        ...
        fogFunction(cityid, fogdata, fogdata2, temperaturedata, humiditydata, winddata, winddirectiondata, current)
...
function fogFunction(cityid, fogdata, fogdata2, temperaturedata, humiditydata, winddata, winddirectiondata, current) {
    var charts = []
    console.log(fogdata)
    charts.push(
        Highcharts.stockChart('Fog', {
                            ...
                            events: {
                                mouseOver: function(e) {
                                    ...
                                    if (charts[0].hoverSeries._i == 0) {
                                        ...
                                        document.getElementById("data-detail").innerHTML = "<h3><div>Pointed Values:</div><div><span style='color:'red''>Visibility:</span> " + store[0].toFixed(2) + " km   <\div><div>Temperature: " + store[1].toFixed(2) + "°C    <\div><div>Relative Humidity: " + store[2].toFixed(2) + " %    <\div><div>Wind: " + store[3].toFixed(2) + "Km/h " + store[4] + "°N" + " <\div></h3>   ";
                                    } else if (charts[0].hoverSeries._i == 1) {
                                        document.getElementById("data-detail").innerHTML = "<h3>Predicted Visibility: " + charts[0].series[1].points[this.index - k].y.toFixed(2) + " km";
                                ...
                                mouseOut: function(e) {
                                    ...
                                    document.getElementById("data-detail").innerHTML = "<h3><div>Current Values:</div><div>Visibility: " + current[0] + " km   <\div><div>Temperature: " + current[1] + "°C    <\div><div>Relative Humidity: " + current[2] + " %    <\div><div>Wind: " + current[3] + "Km/h " + current[4] + "°N" + " <\div></h3>   ";
                ...
                yAxis: [{
                    ...
                    plotLines: [{
                        ...
                        color: '#f9a747',
                        label: {
                            text: '<h5 data-toggle="modal" class="line_allowable btn_toggle" href="#modal-form1"><span>Fog</span></h5>',
                            useHTML: true,
                            style: {
    ...
    charts.push(
        Highcharts.stockChart('Temperature', {
            ...
            xAxis: {
                ...
                min: fogdata[fogdata.length - 1][0] - 86400000
            },
            yAxis: [{
                    min: 0,
                    max: 50,
...
//     document.getElementById('unique').addEventListener(
//         eventType,
//         function(e) {
//             var chart,
//                 point,
