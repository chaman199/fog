...
$sql = "SELECT prediction_time,visibility_km,fog_type  FROM public.six_hour_fog_prediction where prediction_time >= '".$_GET['stime']."' and station_name = '".$_GET['station']."' and model_id = '".$_GET['model']."' order by prediction_time asc";
...
$val = "[";    
    ...
    if (pg_num_rows($result) > 0) {
      while($row = pg_fetch_row($result)) {
        ...
        // echo $row;
        if(strlen($val)!=1)
          $val .= ',['.($row[0]).','.($row[1] === NULL ? 1.00 : $row[1]).','.($row[2]).']';
        else
          $val .= '['.($row[0]).','.($row[1] === NULL ? 1.00 : $row[1]).','.($row[2]).']';
